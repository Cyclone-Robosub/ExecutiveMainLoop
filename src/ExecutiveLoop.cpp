...
public:
  ...
  ExecutiveLoop() : Node("executive_main_node") {
    ...
    if (!std::filesystem::exists(stateFileString)) {
      ...
      stateFile << "Time,Depth(M),Pressure,Angular Velocity X, Angular "
                   "Velocity Y, Angular Velocity Z, Linear Acceleration X, "
                   "Linear Acceleration Y, Linear Acceleration Z, Mag Field X, "
                   "Mag Field Y, Mag Field Z, Theta Roll, Theta Pitch, Theta "
                   "Yaw, Velocity U, Velocity V, Velocity W, North(Meters), "
                   "East(M), West(M), LLA 0, LLA1, LLA2, PWM Data"
                << std::endl;
  ...
  // Feel free to later push these into the Sensors Class, but make sure
  // ExecutiveLoop can still access through memory its needed fields.
  ...
  // First get the PWM Array. Then Allow the duration callback to execute and
  // pair the array with the duration. Then push it onto the queue for
  // ExecuteDecision. Notify every time we allow either Duration or Execute to
  ...
  void executeDecisionLoop() {
    ...
    while (loopIsRunning) {
      ...
      if (isManualEnabled) {
        ...
        std::unique_lock<std::mutex> Manual_Override_Lock(
            Manual_Override_mutex);
        if (isManualOverride) {
        ...
        std::unique_lock<std::mutex> QueuepwmValuesLock(Queue_pwm_mutex,
                                                        std::defer_lock);
        // std::cout << "Got the queue lock, looking at the queue." <<
        // std::endl;
        PWM_cond_change.wait(QueuepwmValuesLock,
                             [this] { return !(sizeQueue == 0); });
        // std::cout << "Something is in the queue." << std::endl;
        // Make sure isCurrnetCommandTimedPWM is only changed by Executive
        // DecisionLoop or else put a mutex on it.
        if (!isCurrentCommandTimedPWM) {
          override();
          std::cout << "Executive Decision: Replace Current PWM Command"
                    << std::endl;
...
private:
  // NOTE: there are going to be a lot of unused variables, please remove in the
  // future.
  ...
  // The current PWM and duration ptr will and should always have a value
  // regardless of what Executive DecisionLoop or Send Thrusters want. However,
  // SendThrusters can "finish" a current PWM and duration and will say that it
  // wants a new command, but it can be the same current PWM if
  // ExecutiveDecision decides so. Executive Decision (on its own thread) will
  // see that SendThrusters is not running a command and give it a new current
  // PWM. This is made so that Executive Decision has the chance to give PWM a
  // new Command if the current one is a timedPWM. In Later uses, the State file
  // should use the current PWM that the Send Thruster is using.
  ...
  // If a task is currently running, stop this task and set a zero pair to clear
  // everything. Execute Decision Loop should then be able to decide what to do
  // next.
  void override() {
    if (isRunningThrusterCommand) {
...
private:
  ...
  rclcpp::Subscription<std_msgs::msg::Int32MultiArray>::SharedPtr
      CLTool_subscription_;
  rclcpp::Subscription<inertial_sense_ros2::msg::DIDINS1>::SharedPtr
      did_ins_subscription_;
